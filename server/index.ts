...
app.post('/api/scrape', async (req: Request, res: Response) => {
	try {
		const {
			...
			bearerToken // New optional bearerToken from request
		}: {
			RARITY?: string;
			CARD_SET?: string;
			FACTION?: string;
			...
			locale?: string;
			bearerToken?: string; // Added to type definition
		} = req.body;
		...
		const tokenToUse = bearerToken || getBearerToken();

		// Validate token presence if not using fallback (optional: throw error if neither is present)
		if (!tokenToUse) {
			console.warn('⚠️ No Bearer token provided in request or .env. Scraper might fail.');
			// Depending on strictness, you might want to return an error here:
			// return res.status(400).json({ success: false, error: 'Bearer token is required.' });
		}

		const scraper = createScraper(locale, tokenToUse);
